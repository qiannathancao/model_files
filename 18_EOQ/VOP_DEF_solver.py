...
class solver():
    ...
    def get_vop_qty(self, pricing_row, vop_day=1):
        ...
        """
        vop_qty = (pricing_row['eau'] / DEF_config.EAU_CALENDER_DAYS) * vop_day
...
if __name__ == '__main__':
    ...
    input_file_path = DEF_config.UPLOAD_FILE_PATH
    input_file_name = DEF_config.UPLOAD_FILE_NAME
    ...
    vop_master_run[['optimal_vop_day', 'optimal_vop_qty', 'optimal_vop_freq', 'optimal_pricing_bucket']] = \
        vop_master_run.apply(lambda row: solver.get_optimal_vops(row,
                                                                 vop_bound=DEF_config.vop_bound,
                                                                 financial_rate=DEF_config.DEF_carrying_financial_rate,
                                                                 pricing_break=pricing_break)
        if row['eau'] != 0 else 0, axis=1, result_type='expand')

    vop_master_run.to_excel(os.path.join(DEF_config.RESULT_SAVE_PATH, '(result)_'+DEF_config.RESULT_FILE_NAME),
                            index=False)
    print(f'Step 4. problem solved, result saved as {DEF_config.RESULT_FILE_NAME}')
