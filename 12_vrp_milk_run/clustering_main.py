...
class ETL_data:
    ...
    def clean_cass(self, source_file, save_file, source_state=None, source_country=None, dest_zip='54942',
        ...
        df = df[['source_weekend_date', 'mode', 'shipper_name',
                 'shipper_address','shipper_city', 'shipper_zip',
                 'shipper_state', 'shipper_country', 'destination_city',
    ...
    def name_convention(self, df, level = config.FUZZY_LEVEL):
        ...
        """
        df['std_name'] = df.shipper_name.str.strip().str.lower().str.replace('-', ''). \
        ...
        while name_list:
            ...
            df_temp['std_name'] = selected_names
            df_temp['unique_name'] = key
    ...
    def period_freq(self, df, freq_list = config.RESAMPLE_LIST, period_level=config.PERIOD_FREQ_LEVEL):
        ...
        for f in freq_list:
            ...
            df_freq = df_ts.groupby(['unique_name', 'shipper_zip'])['ship_weight'].resample(f).agg(
                ['count']).reset_index()
        ...
        # weekly
        s_name_wk = ratio_master[(ratio_master.freq == 'W') & (ratio_master.continuous_shipment >= period_level)][
            'unique_name']
        s_zip_wk = ratio_master[(ratio_master.freq == 'W') & (ratio_master.continuous_shipment >= period_level)][
            'shipper_zip']
        ...
        # biweekly
        s_name_2wk = ratio_master[(ratio_master.freq == '2W') & (ratio_master.continurous_shipment >= period_level)][
            'unique_name']
        s_zip_2wk = ratio_master[(ratio_master.freq == '2W') & (ratio_master.continurous_shipment >= period_level)][
            'shipper_zip']
